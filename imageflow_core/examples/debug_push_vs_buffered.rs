...
fn main() {
    ...
    eprintln!("Buffered: {}x{} {:?} {} bytes", buffered_w, buffered_h, buffered_desc, buffered_bytes.len());
    ...
    let mut push_buf = vec![0xCCu8; stride * buffered_h as usize]; // fill with 0xCC sentinel
    {
        ...
        impl DecodeRowSink for TestSink<'_> {
            fn begin(&mut self, w: u32, h: u32, _desc: zenpixels::PixelDescriptor) -> Result<(), SinkError> {
                self.w = w; self.h = h; Ok(())
            }
            fn provide_next_buffer(&mut self, y: u32, height: u32, width: u32, descriptor: zenpixels::PixelDescriptor)
                -> Result<zenpixels::PixelSliceMut<'_>, SinkError>
            {
                let row_start = y as usize * self.stride;
                let row_bytes = width as usize * 4;
                let needed = if height > 0 { (height as usize - 1) * self.stride + row_bytes } else { 0 };
        ...
        match job2.push_decode(Cow::Borrowed(&jpeg), &mut sink, &preferred) {
            Ok(info) => eprintln!("Push decode OK: {}x{} {:?}", info.width, info.height, info.native_format),
            Err(e) => { eprintln!("Push decode error: {e}"); return; }
        }
    }
        ...
        for x in 0..row_bytes {
            ...
            if d > 0 {
                ...
                if first_diff_y.is_none() {
                    ...
                    eprintln!("First diff at y={y} x={x}: buffered={:#04x} push={:#04x} delta={d}",
    ...
    if sentinel_count > 0 {
        eprintln!("WARNING: {sentinel_count} sentinel bytes (0xCC) in push output — unwritten pixels!");
    }
    ...
    for y in 0..buffered_h as usize {
        ...
        if sentinel > 0 {
            eprintln!("  y={y}: {sentinel}/{row_bytes} sentinel bytes, first 32: {:02x?}", &row[..32.min(row_bytes)]);
        }
    }
}
