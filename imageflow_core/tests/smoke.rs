...
use imageflow_core::{here, Context, Result};
use imageflow_types::*;
    ...
    let execute = Execute001 {
        ...
        framewise: Framewise::Steps(vec![
            Node::CreateCanvas {
                ...
                format,
                color: if with_alpha {
                    Color::Srgb(ColorSrgb::Hex("FF0000CC".to_owned()))
                } else {
                    Color::Srgb(ColorSrgb::Hex("FF0000FF".to_owned()))
                },
            },
            Node::Encode {
                io_id: 1,
                preset: match with_palette {
                    true => EncoderPreset::Pngquant {
                        quality: Some(100),
                        minimum_quality: Some(0),
                        speed: None,
...
pub fn test_format_selection_json_with_source(
    ...
    format_selection_core(
        Some(preset),
        None,
        source_bytes
            .map(|bytes| SourceImageType::ByteArray(bytes.to_vec()))
            .unwrap_or(SourceImageType::CanvasBgra32),
    )
    .map_err(|e| e.at(here!()))
}
...
pub fn riapi_format_selection(command: &str, source: SourceImageType) -> Result<EncodingOutput> {
...
pub fn format_selection_core(
    ...
    if preset.is_some() == command.is_some() {
        return Err(nerror!(
            imageflow_core::ErrorKind::InvalidNodeParams,
            "preset and command must both be present or both be absent"
        ));
    }
    ...
    let steps = if let Some(result_bytes) = generate_source_image(source.clone()) {
        ...
        IoTestTranslator {}.add(&mut context, 0, IoTestEnum::ByteArray(bytes.to_vec())).unwrap();
        ...
        if let Some(command) = command {
            ...
            vec![Node::Decode { io_id: 0, commands: None }, Node::Encode { io_id: 1, preset }]
        } else {
            vec![]
        }
    } else {
        let (color, format) = match source.clone() {
            ...
            SourceImageType::Canvas { color, format } => (color, format),
            _ => unreachable!(),
        };
        // Use canvas (no source)
        vec![
            Node::CreateCanvas { w: 8, h: 8, format, color },
    ...
    match response {
        ResponsePayload::JobResult(job_result) => {
            let diagnostics = job_result
                .encodes
                .first()
                .unwrap()
                .diagnostic_data
                .clone()
                .unwrap_or_else(|| Box::new(BTreeMap::new()));
            // eprintln all diagnostics
            for (key, value) in diagnostics.iter() {
                eprintln!("{}: {}", key, value);
            }
            Ok(EncodingOutput { format: check_magic_bytes(bytes), diagnostics })
        }
        _ => unreachable!(),
    }
}
...
fn generate_source_image(source_image_type: SourceImageType) -> Option<Result<Vec<u8>>> {
    match source_image_type {
        SourceImageType::ByteArray(bytes) => Some(Ok(bytes)),
        SourceImageType::Jpeg => Some(generate_tiny_jpeg()),
        SourceImageType::Png { with_alpha, with_palette } => {
            Some(generate_tiny_png(with_alpha, with_palette))
        }
        SourceImageType::WebP { lossless, alpha } => Some(generate_tiny_webp(alpha, lossless)),
        SourceImageType::AnimatedGif => Some(Ok(TINY_ANIMATED_GIF.to_vec())),
        SourceImageType::Gif { animated } => match animated {
...
fn test_json_format_explicit_avif_with_allow() {
    ...
    let format = preset_format_selection(
        s::EncoderPreset::Format {
            ...
            lossless: None,
            matte: None,
            allow: Some(s::AllowedFormats { avif: Some(true), ..s::AllowedFormats::none() }),
            encoder_hints: None,
        },
        SourceImageType::CanvasBgra32,
    )
    ...
    assert_eq!(format.format, "avif", "Expected AVIF output with format=avif + allow.avif=true");
}
...
fn test_json_format_auto_with_avif_allowed() {
    ...
    let format = preset_format_selection(
        s::EncoderPreset::Auto {
            ...
            lossless: None,
            matte: None,
            allow: Some(s::AllowedFormats { avif: Some(true), ..s::AllowedFormats::web_safe() }),
        },
        SourceImageType::CanvasBgra32,
    )
    .unwrap();

    assert_eq!(format.format, "avif", "Auto format should select AVIF when allow.avif=true");
}
...
fn test_json_format_explicit_webp() {
    ...
    let format = preset_format_selection(
        s::EncoderPreset::Format {
            ...
            lossless: None,
            matte: None,
            allow: Some(s::AllowedFormats { webp: Some(true), ..s::AllowedFormats::none() }),
            encoder_hints: None,
        },
        SourceImageType::CanvasBgra32,
    )
...
fn test_json_format_auto_with_webp_only() {
    ...
    let format = preset_format_selection(
        s::EncoderPreset::Auto {
            ...
            lossless: None,
            matte: None,
            allow: Some(s::AllowedFormats { webp: Some(true), ..s::AllowedFormats::web_safe() }),
        },
        SourceImageType::CanvasBgra32,
    )
    .unwrap();
...
fn test_json_format_auto_for_bgr32_with_default_allow() {
    ...
    assert_eq!(format.format, "jpeg", "Auto format with no allow defined should select JPEG");
}
// ========== RIAPI Tests ==========
...
fn test_riapi_format_avif_with_accept() {
    let format =
        riapi_format_selection("format=avif&accept.avif=1", SourceImageType::CanvasBgra32).unwrap();

    assert_eq!(format.format, "avif", "RIAPI format=avif with accept.avif=1 should produce AVIF");
}

#[test]
fn test_riapi_format_avif_with_accept_false() {
    let format =
        riapi_format_selection("format=avif&accept.avif=0", SourceImageType::CanvasBgra32).unwrap();

    assert_eq!(format.format, "avif", "RIAPI format=avif with accept.avif=0 should produce AVIF");
}
#[test]
...
fn test_riapi_format_auto_with_accept_avif() {
    let format =
        riapi_format_selection("format=auto&accept.avif=1", SourceImageType::CanvasBgra32).unwrap();

    assert_eq!(format.format, "avif", "RIAPI format=auto with accept.avif=1 should select AVIF");
}
...
fn test_riapi_format_auto_with_webp_only() {
    let format =
        riapi_format_selection("format=auto&accept.webp=1", SourceImageType::CanvasBgra32).unwrap();
...
fn test_riapi_format_auto_with_avif_and_webp() {
    let format = riapi_format_selection(
...
fn test_riapi_format_webp_with_quality() {
    let format =
        riapi_format_selection("format=webp&webp.quality=75", SourceImageType::CanvasBgra32)
            .unwrap();
...
fn test_riapi_format_avif_with_quality_and_speed() {
    let format = riapi_format_selection(
        "format=avif&avif.quality=80&avif.speed=6",
        SourceImageType::CanvasBgra32,
    )
    .unwrap();
...
fn test_riapi_auto_from_png_alpha_source() {
    ...
    assert_eq!(format.format, "webp", "RIAPI auto from PNG with alpha should prefer WebP");
}
...
fn test_riapi_auto_from_gif_source() {
    let format = riapi_format_selection(
        "format=auto&accept.avif=1&accept.webp=1",
        SourceImageType::Gif { animated: false },
    )
    .unwrap();
...
fn test_riapi_keep_from_gif_source() {
    ...
    assert_eq!(format.format, "gif", "RIAPI format=keep from GIF source should preserve as GIF");
}
...
static TINY_ANIMATED_GIF: [u8; TINY_ANIMATED_GIF_LEN] = [
    ...
    0x00, 0x00, 0x00, 0x21, 0xf9, 0x04, 0x04, 0x00, 0x00, 0x00, 0x00, 0x21, 0xff, 0x0b, 0x4e, 0x45,
    ...
    ,
