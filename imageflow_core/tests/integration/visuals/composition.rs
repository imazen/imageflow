...
fn test_graph_mode_dual_encode() {
    ...
    ctx.execute_1(Execute001 { graph_recording: None, security: None, framewise: graph }).unwrap();
...
fn test_pyramid_multi_output() {
    ...
    // Node 8: encode 200 → JXL
    nodes.insert(
        "8".to_owned(),
        Node::Encode { io_id: 4, preset: EncoderPreset::JxlLossy { distance: 2.0 } },
    );
    ...
    let jxl_bytes = ctx.take_output_buffer(4).unwrap();
    ...
    assert!(
        jxl_bytes.starts_with(&[0xFF, 0x0A]) || jxl_bytes.starts_with(&[0x00, 0x00, 0x00, 0x0C]),
        "200px output should be JXL"
    );
    ...
    eprintln!(
        "Pyramid outputs: JPEG={}B, WebP={}B, PNG={}B, JXL={}B",
        jpeg_bytes.len(),
        webp_bytes.len(),
        png_bytes.len(),
        jxl_bytes.len()
...
fn test_pyramid_constrain_within() {
    ...
    nodes.insert(
        "8".to_owned(),
        Node::Encode { io_id: 4, preset: EncoderPreset::JxlLossy { distance: 1.5 } },
    );
    ...
    eprintln!(
        "Pyramid (constrain within): JPEG={}B, WebP={}B, PNG={}B, JXL={}B",
        jpeg_bytes.len(),
        webp_bytes.len(),
        png_bytes.len(),
        jxl_bytes.len()
