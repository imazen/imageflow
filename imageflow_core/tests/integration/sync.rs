...
fn sync_and_verify_uploads() {
    ...
    let result = upload_tracker::sync_and_verify(&checksums_dir, &output_cache_dir, &storage);
...
fn verify_upload_log() {
    ...
    println!("Referenced: {} | Uploaded: {} | Missing: {}", all_refs.len(), uploaded.len(), missing.len());
