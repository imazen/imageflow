...
fn function_bounded(
    ...
    input_start_value: f64,
    stop_at_abs: f64,
    input_step: f64,
    result_low_threshold: f64,
    result_high_threshold: f64,
) -> bool {
    ...
    function_bounded(
        details,
        input_value + input_step,
        stop_at_abs,
        input_step,
        result_low_threshold,
        result_high_threshold,
    )
}
...
fn test_filter(
    filter: imageflow_core::graphics::weights::Filter,
    expected_first_crossing: f64,
    expected_second_crossing: f64,
    expected_near0: f64,
    near0_threshold: f64,
    expected_end: f64,
) {
...
fn test_interpolation_filter() {
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::Hermite,
        0f64,
        0f64,
        0.99f64,
        0.08f64,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::Triangle,
        0f64,
        0f64,
        0.99f64,
        0.08f64,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::Box,
        0f64,
        0f64,
        0.51f64,
        0.001f64,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::CatmullRom,
        1f64,
        2f64,
        1f64,
        0.08f64,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::CubicBSpline,
        0f64,
        0f64,
        1.75f64,
        0.08f64,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::Mitchell,
        8.0 / 7.0,
        2.0,
        1f64,
        0.08,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::Robidoux,
        1.1685777620836932,
        2f64,
        1f64,
        0.08,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::RobidouxSharp,
        1.105822933719019,
        2f64,
        1f64,
        0.08,
    ...
    test_filter(
        imageflow_core::graphics::weights::Filter::Lanczos2Sharp,
        0.954,
        1.86,
        1f64,
        0.08,
...
fn test_output_weight() {
    ...
    let scalings: [u32; 44] = [
        /*downscale to 1px*/ 1, 1, 2, 1, 3, 1, 4, 1, 5, 1, 6, 1, 7, 1, 17, 1,
        /*upscale from 2px*/ 2, 3, 2, 4, 2, 5, 2, 17, /*other*/ 11, 7, 7, 3,
    ...
    for (index, &filter) in filters.iter().enumerate() {
        ...
        for i in (0..scalings.len()).step_by(2) {
            ...
            assert_eq!(
                imageflow_core::graphics::weights::populate_weights(
                    &mut w,
                    scalings[i + 1],
                    scalings[i],
                    &details
    ...
    assert_eq!(
        output.trim(),
        include_str!("weights.txt").to_string().trim().replace("\r\n", "\n")
    );
}
    ...
    for &filter in filters.iter() {
        ...
        for i in (0..scalings.len()).step_by(2) {
            ...
            for o_index in 0..w.contrib_row().len() / 2 {
                ...
                assert_eq!(
                    (scalings[i] as i32) - 1 - opposite_output_pixel.right_pixel as i32,
                    output_pixel.left_pixel as i32
                );
                assert_eq!(
                    (scalings[i] as i32) - 1 - output_pixel.right_pixel as i32,
                    opposite_output_pixel.left_pixel as i32
                );
                for (w_index, &weight) in w.weights()
                    [output_pixel.left_weight as usize..=output_pixel.right_weight as usize]
                    .iter()
