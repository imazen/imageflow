...
// ===========================================================================
// Group 9: DiscardGamaChrm decoder command
//
// Verifies that DiscardGamaChrm makes the decoder ignore gAMA+cHRM chunks
// while still honoring iCCP and sRGB chunks.
// ===========================================================================

/// Decode with DiscardGamaChrm and a specific decoder backend.
fn decode_with_discard_gama_chrm(png_bytes: &[u8], decoder: NamedDecoders) -> Vec<u8> {
    let mut ctx = Context::create().unwrap();
    ctx.enabled_codecs.prefer_decoder(decoder);
    ctx.add_input_vector(0, png_bytes.to_vec()).unwrap();
    ctx.add_output_buffer(1).unwrap();
    ...
    let execute = s::Execute001 {
        ...
        security: None,
        framewise: s::Framewise::Steps(vec![
            s::Node::Decode { io_id: 0, commands: Some(vec![s::DecoderCommand::DiscardGamaChrm]) },
            s::Node::Encode {
                io_id: 1,
                preset: s::EncoderPreset::Libpng {
                    depth: Some(s::PngBitDepth::Png32),
...
/// DiscardGamaChrm should NOT affect iCCP — iCCP is still honored.
#[test]
fn test_png_discard_gama_chrm_preserves_iccp() {
    let input = test_gradient();
    let icc = build_gamma_icc_profile(1.8);
    let png = build_test_png(4, 4, &input, &[PngColorChunk::Iccp(icc)]);

    // With DiscardGamaChrm: iCCP should still be applied
    let discarded = decode_with_discard_gama_chrm(&png, NamedDecoders::ImageRsPngDecoder);
    let delta = max_channel_delta(&input, &discarded);
    assert!(
        delta >= 5,
        "discard_gama_chrm_preserves_iccp: delta {delta} (expected ≥ 5). \
         iCCP should still be honored even with DiscardGamaChrm."
    );
    eprintln!("discard_gama_chrm_preserves_iccp: OK (delta={delta})");
}
