...
impl ColorTransformCache {
    ...
    fn create_profile_transform(
        ...
        let p = Profile::new_icc_context(ColorTransformCache::create_thread_context(), bytes)
            .map_err(|e| ColorTransformCache::get_lcms_error(e).at(here!()))?;
        ...
        match color.source {
            ...
            ffi::ColorProfileSource::GAMA_CHRM => {
                ...
                if GAMA_TRANSFORMS.len() > 3 {
                    ...
                    let hash =
                        ColorTransformCache::hash(color, input_pixel_format, output_pixel_format)
                            .unwrap();
                    if !GAMA_TRANSFORMS.contains_key(&hash) {
                        let transform =
                            ColorTransformCache::create_gama_transform(color, pixel_format)
                                .map_err(|e| e.at(here!()))?;
