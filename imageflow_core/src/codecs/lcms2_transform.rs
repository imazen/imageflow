...
impl Lcms2TransformCache {
    ...
    pub fn transform_to_srgb(
        ...
        match profile {
            ...
            SourceProfile::IccProfile(bytes) => {
                Self::transform_icc(frame, bytes, PixelFormat::BGRA_8, PixelFormat::BGRA_8, true)
            }
            SourceProfile::IccProfileGray(bytes) => Self::transform_icc_gray(frame, bytes),
            SourceProfile::CmykIcc(bytes) => {
                // CMYK input: byte 3 is K (black), not alpha. Don't preserve it.
                Self::transform_icc(
                    frame,
                    bytes,
    ...
    fn transform_icc(
        ...
        Self::apply_transform(frame, &t, preserve_alpha)
    }
    ...
    fn apply_gray_transform(
        ...
        let mut alphas: Vec<u32> = vec![0; w];
        for mut line in frame.scanlines_u32()? {
            let row = line.row();
            // Save alpha values (high byte of each BGRA u32 on LE)
            for (a, &px) in alphas.iter_mut().zip(row.iter()) {
                *a = px & 0xFF00_0000;
            }
    ...
    fn transform_gama(
        ...
        Self::apply_transform(frame, &t, true)
    }
    ...
    fn apply_transform(
        ...
        if preserve_alpha {
            ...
            // via BGRA_8 pixel format. We save alpha to a scratch buffer before
            // the in-place transform and restore it afterward.
            let w = frame.w() as usize;
            let mut alphas: Vec<u32> = vec![0; w];
            for mut line in frame.scanlines_u32()? {
            ...
            // the alpha byte undefined (often 0) after CMYK→BGRA transform.
