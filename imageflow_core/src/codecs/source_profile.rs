...
impl SourceProfile {
    ...
    /// Returns `Srgb` for profiles too short to be valid ICC (< 132 bytes header).
    ...
    /// Discard gAMA+cHRM information, treating it as absent.
    /// Returns `Srgb` if this profile was derived from gAMA+cHRM chunks;
    /// other profile types (ICC, CICP, etc.) are passed through unchanged.
    pub fn without_gama_chrm(self) -> Self {
        match self {
            SourceProfile::GammaPrimaries { .. } => SourceProfile::Srgb,
            other => other,
        }
    }
