...
impl InterpolationDetails {
    ...
    pub fn create(filter: Filter) -> InterpolationDetails {
        match filter {
            Filter::Triangle | Filter::Linear => InterpolationDetails {
                window: 1f64,
                blur: 1f64,
                filter: filter_triangle,
                ..Default::default()
            },

            Filter::RawLanczos2 => InterpolationDetails {
                window: 2f64,
                blur: 1f64,
                filter: filter_sinc,
                ..Default::default()
            },
            Filter::RawLanczos3 => InterpolationDetails {
                window: 3f64,
                blur: 1f64,
                filter: filter_sinc,
                ..Default::default()
            },
            Filter::RawLanczos2Sharp => InterpolationDetails {
                window: 2f64,
                blur: 0.9549963639785485f64,
                filter: filter_sinc,
                ..Default::default()
            },
            Filter::RawLanczos3Sharp => InterpolationDetails {
                window: 3f64,
                blur: 0.9812505644269356f64,
                filter: filter_sinc,
                ..Default::default()
            },
            Filter::Lanczos2 => InterpolationDetails {
                window: 2f64,
                blur: 1f64,
                filter: filter_sinc_windowed,
                ..Default::default()
            },
            Filter::Lanczos => InterpolationDetails {
                window: 3f64,
                blur: 1f64,
                filter: filter_sinc_windowed,
                ..Default::default()
            },
            Filter::Lanczos2Sharp => InterpolationDetails {
                window: 2f64,
                blur: 0.9549963639785485f64,
                filter: filter_sinc_windowed,
                ..Default::default()
            },
            Filter::LanczosSharp => InterpolationDetails {
                window: 3f64,
                blur: 0.9812505644269356f64,
                filter: filter_sinc_windowed,
                ..Default::default()
            },
            Filter::CubicFast => InterpolationDetails {
                window: 2f64,
                blur: 1f64,
                filter: filter_bicubic_fast,
                ..Default::default()
            },
            Filter::Box => InterpolationDetails {
                window: 0.5f64,
                blur: 1f64,
                filter: filter_box,
                ..Default::default()
            },
            Filter::Ginseng => InterpolationDetails {
                window: 3f64,
                blur: 1f64,
                filter: filter_ginseng,
                ..Default::default()
            },
            Filter::GinsengSharp => InterpolationDetails {
                window: 3f64,
                blur: 0.9812505644269356f64,
                filter: filter_ginseng,
                ..Default::default()
            },
            Filter::Jinc => InterpolationDetails {
                window: 6f64,
                blur: 1f64,
                filter: filter_jinc,
                ..Default::default()
            ...
            Filter::CubicSharp => {
                InterpolationDetails::bicubic(2f64, 0.9549963639785485f64, 0f64, 1f64)
            }
            ...
            Filter::CatmullRomFastSharp => {
                InterpolationDetails::bicubic(1f64, 13.0f64 / 16.0f64, 0f64, 0.5f64)
            }
            ...
            Filter::NCubic => InterpolationDetails::bicubic(
                2.5f64,
                1.0f64 / 1.1685777620836933f64,
                0.3782157550939987f64,
                0.3108921224530007f64,
            ),

            Filter::NCubicSharp => InterpolationDetails::bicubic(
                2.5f64,
            ...
            Filter::Robidoux => InterpolationDetails::bicubic(
                2f64,
                1.0f64,
                0.3782157550939987f64,
                0.3108921224530007f64,
            ...
            Filter::LegacyIDCTFilter => InterpolationDetails::bicubic(
                2f64,
                1. / 1.1685777620836932,
                0.3782157550939987f64,
                0.3108921224530007f64,
            ),

            Filter::Fastest => InterpolationDetails::bicubic(
                0.74f64,
                0.74f64,
                0.3782157550939987f64,
                0.3108921224530007f64,
            ...
            Filter::Hermite => InterpolationDetails::bicubic(1f64, 1f64, 0f64, 0f64),
        }
    }

    fn calculate_percent_negative_weight(&self) -> f64 {
        let samples: i32 = 50i32;
        let step: f64 = self.window / samples as f64;
        let mut last_height: f64 = (self.filter)(self, -step);
        let mut positive_area: f64 = 0i32 as f64;
        let mut negative_area: f64 = 0i32 as f64;
        for i in 0..(samples + 3i32) {
            let height: f64 = (self.filter)(self, i as f64 * step);
...
fn filter_bicubic_fast(d: &InterpolationDetails, t: f64) -> f64 {
    ...
    if abs_t < 1f64 {
        1f64 - 2f64 * abs_t_sq + abs_t_sq * abs_t
    } else if abs_t < 2_f64 {
        4f64 - 8f64 * abs_t + 5f64 * abs_t_sq - abs_t_sq * abs_t
    } else {
        0f64
    }
}
...
fn filter_sinc_windowed(d: &InterpolationDetails, t: f64) -> f64 {
    ...
    if abs_t == 0i32 as f64 {
        ...
        d.window * (f64::consts::PI * x / d.window).sin() * (x * f64::consts::PI).sin()
...
fn filter_ginseng(d: &InterpolationDetails, t: f64) -> f64 {
    ...
    if abs_t == 0f64 {
        ...
        let jinc_input: f64 = 1.2196698912665046f64 * t_pi / d.window;
        let jinc_output: f64 = bessj1(jinc_input) / (jinc_input * 0.5f64);
...
pub trait WeightContainer {
    ...
    fn try_reserve(
        &mut self,
        total_output_pixels: u32,
        inputs_per_outputs: u32,
    ) -> Result<(), WeightsError>;
    fn add_output_pixel(
        &mut self,
        left_input_pixel: u32,
        right_input_pixel: u32,
        weights: &[f32],
    ) -> Result<(), WeightsError>;
}
...
impl PixelRowWeights {
    ...
    pub fn contrib_row(&self) -> &[PixelWeightIndexes] {
        self.contrib_row.as_ref().map(|r| &r.as_slice()[..self.contrib_row_length]).unwrap_or(&[])
    }
    pub fn weights(&self) -> &[f32] {
        self.weights.as_ref().map(|r| &r.as_slice()[..self.weights_length]).unwrap_or(&[])
    }
    ...
    pub fn create_for(
        ...
        input_line_size: u32,
    ) -> Result<PixelRowWeights, WeightsError> {
...
impl WeightContainer for PixelRowWeights {
    fn try_reserve(
        ...
        if self.contrib_row.is_some() || self.weights.is_some() {
            return Err(WeightsError::TryReserveCalledRepeatedly);
        }
        self.contrib_row = Some(
            AlignedBuffer::new(total_output_pixels as usize, 16)
                .map_err(|e| WeightsError::AllocationFailed)?,
        );

        self.weights = Some(
            AlignedBuffer::new(inputs_per_outputs as usize * total_output_pixels as usize, 64)
                .map_err(|e| WeightsError::AllocationFailed)?,
        );
    ...
    fn add_output_pixel(
        ...
        if self.contrib_row.is_none() || self.weights.is_none() {
...
impl WeightContainer for PixelRowWeightsSimple {
    ...
    fn add_output_pixel(
        ...
        if self.contrib_row.len() < self.contrib_row.capacity() {
            self.contrib_row.push(PixelWeightsSimple {
                weights: weights.to_vec(),
                left: left_input_pixel as i32,
                right: right_input_pixel as i32,
            });
            Ok(())
        } else {
...
pub fn populate_weights<T: WeightContainer>(
    container: &mut T,
    output_line_size: u32,
    input_line_size: u32,
    details: &InterpolationDetails,
    ...
    let allocated_window_size: u32 =
        ((2_f64 * (half_source_window - 0.00001f64)).ceil() as i32 + 1i32) as u32;
    ...
    for u in 0..output_line_size {
        ...
        let left_edge: i32 =
            (center_src_pixel - details.window / downscale_factor - 0.0001).ceil() as i32;
        let right_edge: i32 =
            (center_src_pixel + details.window / downscale_factor + 0.0001).floor() as i32;
        let left_src_pixel: u32 = 0i32.max(left_edge) as u32;
        let right_src_pixel: u32 = right_edge.min(input_line_size as i32 - 1i32) as u32;
        // Net weight
        ...
        let source_pixel_count: u32 =
            right_src_pixel.wrapping_sub(left_src_pixel).wrapping_add(1i32 as u32);
        ...
        for ix in left_src_pixel..=right_src_pixel {
            ...
            let mut add: f64 =
                filter_func(details, downscale_factor * (ix as f64 - center_src_pixel));
            if add.abs() <= 2e-8f64 {
                add = 0.0f64
            }
