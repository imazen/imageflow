...
impl<'a,T> MethodRouter<'a, T>{
    ...
    pub fn add(&mut self, method: &'static str, handler: MethodHandler<'a, T>) -> Option<MethodHandler<'a, T>>{
        self.method_names.push(method);
        self.handlers.insert(method, handler)
    }

    pub fn add_responder<D>(&mut self, method: &'static str, responder: ResponderFn<'a, T,D>) -> Option<MethodHandler<'a, T>> where D: serde::Deserialize, D: 'a, T: 'a{
        self.method_names.push(method);
    ...
    ///
    pub fn invoke(&self, upon: &mut T, method: &str, json_request_body: &[u8]) -> Result<JsonResponse>{
...
pub fn create_handler_over_responder<'a, T,D>(responder: ResponderFn<'a, T,D>) -> MethodHandler<'a, T> where D: serde::Deserialize, D: 'a, T: 'a{
    Box::new( move | upon: &mut T, json_request_bytes: & [u8] | {

        let parsed_maybe: std::result::Result<D, serde_json::Error> = serde_json::from_slice(json_request_bytes);
        match parsed_maybe {
            Ok(parsed) => {
                ...
                match payload_maybe{
                    ...
                    Err(error) => {
                        Ok(JsonResponse::fail_with_message(500, "error translation not implemented"))
                    }
                }
            }
            Err(e) => {
                Ok(JsonResponse::from_parse_error(e, json_request_bytes))
            }
        }
...
impl JsonResponse {
    ...
    pub fn fail_with_message(code: i64, message: &str) -> JsonResponse{
        JsonResponse {
            status_code: 404,
            response_json: Cow::Owned(format!("{}\"success\": \"false\",\"code\": {},\"message\": {:?}{}","{", code, message, "}").into_bytes())
        }
    }
