...
impl LibClient {
    ...
    pub fn get_image_info(
        ...
        let result = catch_unwind(AssertUnwindSafe(|| {
            LibClient::get_image_info_inner(&mut context, bytes)
                .map_err(|e| BuildFailure::from(e.at(here!())))
        }));

        let result = match result {
            Err(panic) => Err(BuildFailure::Error {
                httpish_code: 500,
                message: format!("{}", PanicFormatter(&panic)),
            }),
            Ok(Err(e)) => Err(e),
            Ok(Ok(v)) => Ok(v),
        };
    ...
    pub fn build(&mut self, task: BuildRequest) -> std::result::Result<BuildSuccess, BuildFailure> {
        ...
        let result = catch_unwind(AssertUnwindSafe(|| {
            LibClient::build_inner(&mut context, task)
                .map_err(|e| BuildFailure::from(e.at(here!())))
        }));
...
fn test_stateless() {
    ...
    let req = BuildRequest {
        inputs: vec![BuildInput { io_id: 0, bytes: &png_bytes }],
