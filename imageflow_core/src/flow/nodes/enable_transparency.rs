...
use imageflow_types::{CompositingMode, PixelFormat};
...
impl NodeDefOneInputExpand for EnableTransparencyDef {
    ...
    fn expand(
        ...
        if parent.fmt == PixelFormat::Bgra32 {
            ...
            let mutate = ctx.graph.add_node(Node::n(&ENABLE_TRANSPARENCY_MUT, NodeParams::None));
            ...
            let copy = ctx.graph.add_node(Node::n(&COPY_RECT, NodeParams::Json(copy_rect_params)));
    ...
    fn mutate(&self, c: &Context, bitmap_key: BitmapKey, p: &NodeParams) -> Result<()> {
        ...
        if bitmap_bitmap.info().alpha_meaningful() {
            Err(nerror!(
                ...
                "Need Bgr32 input image to convert to bgra32"
            ))
        } else {
