...
pub static WHITE_BALANCE_SRGB: MutProtect<WhiteBalanceSrgbMutDef> =
    MutProtect { node: &WHITE_BALANCE_SRGB_MUTATE, fqn: "imazen.white_balance_srgb" };
...
fn apply_mappings(
    bitmap: &mut BitmapWindowMut<Bgra<u8, u8>>,
    map_red: &[u8],
    map_green: &[u8],
    map_blue: &[u8],
) -> Result<()> {
...
fn white_balance_srgb_mut(
    ...
    let (red_low, red_high) =
        area_threshold(&histograms[0], pixels_sampled, low_threshold, high_threshold);
    let (green_low, green_high) =
        area_threshold(&histograms[1], pixels_sampled, low_threshold, high_threshold);
    let (blue_low, blue_high) =
        area_threshold(&histograms[2], pixels_sampled, low_threshold, high_threshold);
...
impl NodeDefMutateBitmap for WhiteBalanceSrgbMutDef {
    ...
    fn mutate(&self, c: &Context, bitmap_key: BitmapKey, p: &NodeParams) -> Result<()> {
        ...
        populate_histogram_from_window(&mut window, &mut histograms).map_err(|e| e.at(here!()))?;
