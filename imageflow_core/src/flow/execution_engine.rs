use super::visualize::{notify_graph_changed, GraphRecordingInfo, GraphRecordingUpdate};
...
impl<'a> Engine<'a> {
    pub fn create(context: &'a mut Context, g: Graph) -> Engine<'a> {
        Engine { c: context, g, more_frames: false }
    }

    pub fn ctx(&self) -> OpCtx<'_> {
        OpCtx { c: self.c, graph: &self.g }
    }
    ...
    pub fn validate_graph(&self) -> Result<()> {
        for node_index in (0..self.g.node_count()).map(NodeIndex::new) {
            ...
            let outbound_count =
                self.g.graph().edges_directed(node_index, EdgeDirection::Outgoing).count();

            let input_count = self
                .g
                .graph()
                .edges_directed(node_index, EdgeDirection::Incoming)
                .filter(|&e| e.weight() == &EdgeKind::Input)
                .count();
            ...
            let result = if let Err(e) = n.def.validate_params(&n.params) {
                ...
                Err(nerror!(
                    ...
                    "Node type {} requires {:?}, but had {} inputs, {} canvases.",
                    n.def.name(),
                    req_edges_in,
                    input_count,
                    canvas_count
                ...
                Err(nerror!(
                    crate::ErrorKind::InvalidNodeConnections,
                    "Node type {} prohibits child nodes, but had {} outbound edges.",
                    n.def.name(),
                    outbound_count
                ))
            } else {
    ...
    fn execute(&mut self) -> Result<(bool, s::FramePerformance)> {
        ...
        let mut perf: Vec<s::NodePerf> = self
            .g
            .node_weights_mut()
            .map(|n| s::NodePerf {
                wall_microseconds: (n.cost.wall_ns as f64 / 1000f64).round() as u64,
                name: n.def.name().to_owned(),
            })
            .collect();
        ...
        Ok((
            self.more_frames,
            s::FramePerformance { nodes: perf, wall_microseconds, overhead_microseconds },
        ))
    }
    ...
    pub fn request_decoder_commands(&mut self) -> Result<()> {
        ...
        for index in 0..self.g.node_count() {
            let result = {
                let n = self.g.node_weight(NodeIndex::new(index)).unwrap();
    ...
    pub fn invalidate_all_graph_estimates(&mut self) -> Result<()> {
        for index in 0..self.g.node_count() {
            self.g.node_weight_mut(NodeIndex::new(index)).unwrap().frame_est = FrameEstimate::None;
        }
        Ok(())
    }
    ...
    fn notify_graph_complete(&mut self) -> Result<()> {
        if self.c.next_graph_version > 0
            && self.c.graph_recording.record_graph_versions.unwrap_or(false)
        {
        ...
        if let Some(frame_info) = info {
            ...
            if max_frame_size.w.leading_zeros() == 0 || max_frame_size.h.leading_zeros() == 0 {
                return Err(nerror!(
                    ...
                    "max_frame_size values overflow an i32"
                ));
            }
            if frame_info.w > max_frame_size.w as i32 {
                return Err(nerror!(
                    ErrorKind::SizeLimitExceeded,
                    "Frame width {} exceeds max_frame_size.w {}",
                    frame_info.w,
                    max_frame_size.w
                ));
            }
            if frame_info.h > max_frame_size.h as i32 {
                return Err(nerror!(
                    ErrorKind::SizeLimitExceeded,
                    "Frame height {} exceeds max_frame_size.h {}",
                    frame_info.h,
                    max_frame_size.h
                ));
            }
            ...
            if megapixels > max_frame_size.megapixels {
                return Err(nerror!(
                    ErrorKind::SizeLimitExceeded,
                    "Frame megapixels {} exceeds max_frame_size.megapixels {}",
                    megapixels,
                    max_frame_size.megapixels
                ));
            }
        }
        Ok(())
    ...
    pub fn estimate_node_recursive(
        &mut self,
        node_id: NodeIndex,
        recurse_limit: i32,
    ...
    fn graph_pre_optimize_flatten(&mut self) -> Result<()> {
        ...
        loop {
            ...
            for ix in 0..(self.g.node_count()) {
                ...
                let def = self.g.node_weight(nix).unwrap().def;
                if def.can_expand() && self.parents_complete(nix) {
                    if let FrameEstimate::Some(_) = self.g.node_weight(nix).unwrap().frame_est {
                    } else {
                        //Try estimation one last time if it didn't happen yet
                        let _ =
                            self.estimate_node_recursive(nix, 100).map_err(|e| e.at(here!()))?;
                    }
                    next = Some((nix, def));
                    break;
                }
                ...
                Some((next_ix, def)) => {
                    let more_frames = {
                        ...
                        def.expand(&mut ctx, next_ix)
                            .map_err(|e| e.with_ctx_mut(&ctx, next_ix).at(here!()))?;
                        ctx.more_frames.get()
                    };
    ...
    pub fn graph_execute(&mut self) -> Result<()> {
        ...
        loop {
            ...
            for ix in 0..(self.g.node_count()) {
                ...
                if def.can_execute() {
                    if self.g.node_weight(index).unwrap().result == NodeResult::None
                        && self.parents_complete(index)
                    {
                        if self.g.node_weight(index).unwrap().frame_est.is_none() {
                            let _ = self
                                .estimate_node_recursive(index, 100)
                                .map_err(|e| e.at(here!()))?;
                        }
                        next = Some((index, def));
                        break;
                    ...
                    return Err(nerror!(
                        crate::ErrorKind::MethodNotImplemented,
                        "Nodes must can_execute() or can_expand(). {:?} does neither",
                        def
                    ));
                }
            }
            ...
            match next {
                ...
                Some((next_ix, def)) => {
                    let more_frames = {
                        ...
                        if result == NodeResult::None {
                            ...
                            if let NodeResult::Frame(bitmap_key) = result {
                                ...
                                ctx.weight_mut(next_ix).frame_est =
                                    FrameEstimate::Some(bitmap_frame_info);
                            }
                    ...
                    if self.c.graph_recording.record_frame_images.unwrap_or(false) {
                        if let NodeResult::Frame(bitmap_key) =
                            self.g.node_weight(next_ix).unwrap().result
                        {
                            let path = format!(
                                "node_frames/job_{}_node_{}.png",
                                self.c.debug_job_id,
                                self.g.node_weight(next_ix).unwrap().stable_id
                            );
                            let _ = std::fs::create_dir("node_frames");

                            let bitmaps = self.c.borrow_bitmaps().map_err(|e| e.at(here!()))?;
                            let mut bitmap =
    ...
    fn op_ctx_mut(&mut self) -> OpCtxMut<'_> {
        OpCtxMut { c: self.c, graph: &mut self.g, more_frames: Cell::new(false) }
    }
