...
impl NodeDebugInfo {
    fn from_ctx(ctx: &OpCtx, ix: NodeIndex) -> Option<NodeDebugInfo> {
        ctx.graph.node_weight(ix).map(|w| NodeDebugInfo {
...
pub enum EdgesIn {
    ...
    OneOptionalInput,
    OneInputOneCanvas,
    Arbitrary { inputs: i32, canvases: i32, infos: i32 },
}
...
pub trait NodeDef: ::std::fmt::Debug {
    ...
    fn name(&self) -> &'static str {
        self.fqn()
            .split_terminator('.')
            .next_back()
            .expect("Node fn fqn() was empty. Value is required.")
    }
    ...
    fn expand(&self, ctx: &mut OpCtxMut, ix: NodeIndex) -> Result<()> {
        if let Some(n) = self.as_one_input_expand() {
            ...
            n.expand(ctx, ix, params, parent).map_err(|e| e.at(here!()))
        } else if let Some(n) = self.as_one_input_one_canvas_expand() {
            let input = ctx.frame_info_from(ix, EdgeKind::Input)?;
            let canvas = ctx.frame_info_from(ix, EdgeKind::Canvas)?;
            let params = ctx.weight(ix).params.clone();
            n.expand(ctx, ix, params, input, canvas).map_err(|e| e.at(here!()))
        } else {
            Err(unimpl!())
        }
    }
    ...
    fn execute(&self, ctx: &mut OpCtxMut, ix: NodeIndex) -> Result<NodeResult> {
        if let Some(n) = self.as_one_input_one_canvas() {
            ...
            n.render(ctx.c, canvas, input, &ctx.weight(ix).params)
    ...
    T: NodeDef,
{
    pub fn new(with: &'static T, fqn: &'static str) -> MutProtect<T> {
        MutProtect { node: with, fqn }
    }
}
impl<T> NodeDef for MutProtect<T>
where
    T: NodeDef,
{
    fn as_one_input_expand(&self) -> Option<&dyn NodeDefOneInputExpand> {
...
impl<T> NodeDefOneInputExpand for MutProtect<T>
where
    T: NodeDef,
{
    fn fqn(&self) -> &'static str {
...
impl FrameEstimate {
    ...
    pub fn map_frame<F>(self, f: F) -> Result<FrameEstimate>
    where
        F: Fn(FrameInfo) -> Result<FrameInfo>,
    {
        match self {
            FrameEstimate::Some(info) => Ok(FrameEstimate::Some(f(info)?)),
            FrameEstimate::UpperBound(info) => Ok(FrameEstimate::UpperBound(f(info)?)),
            other => Ok(other),
        }
    }

    pub fn transpose(&self) -> FrameEstimate {
        self.map_frame(|info| Ok(FrameInfo { w: info.h, h: info.w, fmt: info.fmt })).unwrap()
    }
}
...
pub enum NodeResult {
    ...
    /// Analysis result (focus regions, saliency maps, etc.)
    Analyzed(s::AnalyzeResult),
}
#[derive(Clone, Debug, PartialEq)]
...
impl From<s::Node> for Node {
    fn from(node: s::Node) -> Node {
        match node {
            ...
            s::Node::CreateCanvas { .. } => Node::n(&nodes::CREATE_CANVAS, NodeParams::Json(node)),
            s::Node::RegionPercent { .. } => {
                Node::n(&nodes::REGION_PERCENT, NodeParams::Json(node))
            }
            s::Node::RoundImageCorners { .. } => {
                Node::n(&nodes::ROUND_IMAGE_CORNERS, NodeParams::Json(node))
            }
            s::Node::Region { .. } => Node::n(&nodes::REGION, NodeParams::Json(node)),
            s::Node::CopyRectToCanvas { .. } => Node::n(&nodes::COPY_RECT, NodeParams::Json(node)),
            ...
            s::Node::DrawImageExact { .. } => {
                Node::n(&nodes::DRAW_IMAGE_EXACT, NodeParams::Json(node))
            }
            s::Node::Constrain(_) => Node::n(&nodes::CONSTRAIN, NodeParams::Json(node)),
            s::Node::ExpandCanvas { .. } => Node::n(&nodes::EXPAND_CANVAS, NodeParams::Json(node)),
            ...
            s::Node::Analyze { .. } => Node::n(&nodes::ANALYZE, NodeParams::Json(node)),
        }
    }
}
...
impl Node {
    ...
    pub fn n(def: &'static dyn NodeDef, params: NodeParams) -> Node {
        Node {
            ...
            cost: CostInfo { cpu_ticks: None, wall_ns: 0, heap_bytes: 0, peak_temp_bytes: 0 },
            stable_id: -1,
            params,
