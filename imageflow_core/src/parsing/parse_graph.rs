...
impl GraphTranslator {
    ...
    unsafe fn create_node(&self, g: *mut *mut ::ffi::Graph, node: s::Node) -> i32 {
        match node {
            ...
            s::Node::Decode { io_id } => ::flow::graph::flow_node_create_decoder(self.ctx, g, -1, io_id),
            s::Node::Encode { io_id, encoder_id, encoder, hints: _ } => {
                ...
                ::flow::graph::flow_node_create_encoder(self.ctx,
                                                g,
                                                -1,
                                                io_id,
                                                encoder_id,
            ...
            s::Node::Crop { x1, y1, x2, y2 } => {
                ::flow::graph::flow_node_create_primitive_crop(self.ctx, g, -1, x1, y1, x2, y2)
            }
            s::Node::FlipV => ::flow::graph::flow_node_create_primitive_flip_vertical(self.ctx, g, -1),
            s::Node::FlipH => ::flow::graph::flow_node_create_primitive_flip_horizontal(self.ctx, g, -1),
            s::Node::Rotate90 => ::flow::graph::flow_node_create_rotate_90(self.ctx, g, -1),
            s::Node::Rotate180 => ::flow::graph::flow_node_create_rotate_180(self.ctx, g, -1),
            s::Node::Rotate270 => ::flow::graph::flow_node_create_rotate_270(self.ctx, g, -1),
            s::Node::CreateCanvas { format, w, h, color } => {
                ...
                ::flow::graph::flow_node_create_canvas(self.ctx,
                                               g,
                                               -1,
                                               ffi_format,
                                               w,
            ...
            s::Node::CopyRectToCanvas { from_x, from_y, width, height, x, y } => {
                ::flow::graph::flow_node_create_primitive_copy_rect_to_canvas(self.ctx,
                                                                      g,
                                                                      -1,
                                                                      from_x,
                                                                      from_y,
            ...
            s::Node::Transpose => ::flow::graph::flow_node_create_transpose(self.ctx, g, -1),
            s::Node::ExpandCanvas { left, top, right, bottom, color } => {
                ::flow::graph::flow_node_create_expand_canvas(self.ctx,
                                                      g,
                                                      -1,
                                                      left,
                                                      top,
            ...
            s::Node::Scale{ w, h, down_filter, up_filter,
                ...
                ::flow::graph::flow_node_create_scale(self.ctx, g, -1, w, h, down_filter.unwrap_or(s::Filter::RobidouxSharp) as i32, up_filter.unwrap_or(s::Filter::Ginseng) as i32,  flags.unwrap_or(1), sharpen_percent.unwrap_or(0f32) )
            }
            s::Node::FillRect { x1, x2, y1, y2, color } => {
                ::flow::graph::flow_node_create_fill_rect(self.ctx,
                                                  g,
                                                  -1,
                                                  x1,
                                                  y1,
    ...
    unsafe fn create_edge(&self,
                          ...
                          -> i32 {
        ::flow::graph::flow_edge_create(self.ctx, g, from_node, to_node, edge_kind)
    }


    pub unsafe fn translate_graph(&self, from: s::Graph) -> *mut ::ffi::Graph {
        let mut g = ::flow::graph::flow_graph_create(self.ctx, 10, 10, 3000, 2.0f32);
