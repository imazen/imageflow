...
impl IoStore {
    ...
    pub fn add_input(&mut self, io_id: i32, data: Arc<[u8]>) {
        self.objects
            .insert(io_id, IoProxy { direction: IoDirection::In, data: IoData::Input(data) });
    }
    ...
    pub fn add_output(&mut self, io_id: i32) {
        self.objects.insert(
            io_id,
            IoProxy { direction: IoDirection::Out, data: IoData::Output(Vec::new()) },
        );
    }
    ...
    pub fn get_input(&self, io_id: i32) -> Result<&[u8], FlowError> {
        match self.objects.get(&io_id) {
            Some(IoProxy { data: IoData::Input(data), .. }) => Ok(data),
            Some(_) => Err(FlowError::InvalidPipeline(format!("io_id {io_id} is not an input"))),
    ...
    pub fn write_output(&mut self, io_id: i32, data: Vec<u8>) -> Result<(), FlowError> {
        match self.objects.get_mut(&io_id) {
            Some(IoProxy { data: IoData::Output(buf), .. }) => {
                *buf = data;
                Ok(())
            }
            Some(_) => Err(FlowError::InvalidPipeline(format!("io_id {io_id} is not an output"))),
            None => Err(FlowError::IoNotFound(io_id)),
        }
    }
    ...
    pub fn get_output(&self, io_id: i32) -> Result<&[u8], FlowError> {
        match self.objects.get(&io_id) {
            Some(IoProxy { data: IoData::Output(buf), .. }) => Ok(buf),
            Some(_) => Err(FlowError::InvalidPipeline(format!("io_id {io_id} is not an output"))),
            None => Err(FlowError::IoNotFound(io_id)),
        }
    }
    ...
    pub fn take_output(&mut self, io_id: i32) -> Result<Vec<u8>, FlowError> {
        match self.objects.get_mut(&io_id) {
            Some(IoProxy { data: IoData::Output(buf), .. }) => Ok(std::mem::take(buf)),
            Some(_) => Err(FlowError::InvalidPipeline(format!("io_id {io_id} is not an output"))),
            None => Err(FlowError::IoNotFound(io_id)),
        }
    }
