...
impl AllowedFormats {
    ...
    pub fn png() -> Self {
        AllowedFormats { png: Some(true), ..AllowedFormats::none() }
    }
    ...
    pub fn gif() -> Self {
        AllowedFormats { gif: Some(true), ..AllowedFormats::none() }
    }
    pub fn webp() -> Self {
        AllowedFormats { webp: Some(true), color_profiles: Some(true), ..AllowedFormats::none() }
    }
    pub fn jxl() -> Self {
        AllowedFormats { jxl: Some(true), color_profiles: Some(true), ..AllowedFormats::none() }
    }
    pub fn avif() -> Self {
        AllowedFormats { avif: Some(true), color_profiles: Some(true), ..AllowedFormats::none() }
    }
...
impl Color {
    ...
    pub fn to_color_32(&self) -> std::result::Result<Color32, ParseColorError> {
        match *self {
            Color::Srgb(ref srgb) => match *srgb {
                ColorSrgb::Hex(ref hex_srgb) => parse_color_hex(hex_srgb),
            },
...
fn test_color() {
    assert_eq_hex(
        Color::Srgb(ColorSrgb::Hex("FFAAEEDD".to_owned())).to_u32_rgba_big_endian().unwrap(),
        0xFFAAEEDD,
    );
    assert_eq_hex(
        Color::Srgb(ColorSrgb::Hex("FFAAEE".to_owned())).to_u32_rgba_big_endian().unwrap(),
        0xFFAAEEFF,
    );
}
...
fn test_bgra() {
    assert_eq_hex(
        Color::Srgb(ColorSrgb::Hex("FFAAEEDD".to_owned())).to_color_32().unwrap().to_bgra_le(),
        0xDDFFAAEE,
    );
    assert_eq_hex(
        Color::Srgb(ColorSrgb::Hex("FFAAEE".to_owned())).to_color_32().unwrap().to_bgra_le(),
        0xFFFFAAEE,
    );
    assert_eq_hex(
        Color::Srgb(ColorSrgb::Hex("000000FF".to_owned())).to_color_32().unwrap().to_bgra_le(),
        0xFF000000,
    );
}
...
impl ResampleHints {
    ...
    pub fn with_bi_filter(self, filter: Filter) -> ResampleHints {
        ResampleHints { down_filter: Some(filter), up_filter: Some(filter), ..self }
    }
    pub fn with_floatspace(self, space: ScalingFloatspace) -> ResampleHints {
        ResampleHints { scaling_colorspace: Some(space), ..self }
    }
...
pub enum Node {
    ...
    CopyRectToCanvas { from_x: u32, from_y: u32, w: u32, h: u32, x: u32, y: u32 },
    #[serde(rename = "round_image_corners")]
    RoundImageCorners { radius: RoundCornersMode, background_color: Color },
    #[serde(rename = "decode")]
    Decode { io_id: i32, commands: Option<Vec<DecoderCommand>> },
    #[serde(rename = "encode")]
    Encode { io_id: i32, preset: EncoderPreset },
    ...
    RegionPercent { x1: f32, y1: f32, x2: f32, y2: f32, background_color: Color },
    #[serde(rename = "region")]
    Region { x1: i32, y1: i32, x2: i32, y2: i32, background_color: Color },
    ...
    Resample2D { w: u32, h: u32, hints: Option<ResampleHints> },
    ...
    WhiteBalanceHistogramAreaThresholdSrgb { threshold: Option<f32> },
...
impl IoEnum {
    pub fn into_input(self, io_id: i32) -> IoObject {
        IoObject { io_id, direction: IoDirection::In, io: self }
    }
    pub fn into_output(self, io_id: i32) -> IoObject {
        IoObject { io_id, direction: IoDirection::Out, io: self }
    }
}
...
impl Build001 {
    pub fn example_with_steps() -> Build001 {
        Build001 {
            builder_config: None,
            io: vec![
                IoObject { direction: IoDirection::In, io_id: 0, io: IoEnum::Placeholder },
                IoObject { direction: IoDirection::In, io_id: 92, io: IoEnum::example_base64() },
                IoObject {
                ...
                IoObject { io: IoEnum::OutputBuffer, io_id: 2, direction: IoDirection::Out },
...
impl Framewise {
    pub fn example_steps() -> Framewise {
        Framewise::Steps(vec![
            Node::Decode {
                io_id: 0,
                commands: Some(vec![DecoderCommand::JpegDownscaleHints(JpegIDCTDownscaleHints {
                    width: 800,
                    height: 600,
    ...
    pub fn example_graph() -> Framewise {
        ...
        nodes.insert(
            "2".to_owned(),
            Node::CopyRectToCanvas { x: 0, y: 0, from_x: 0, from_y: 0, w: 100, h: 100 },
        );
        nodes.insert(
            "3".to_owned(),
            Node::Resample2D {
        ...
        Framewise::Graph(Graph {
            edges: vec![
                ...
                Edge { from: 3, to: 5, kind: EdgeKind::Input },
            ],
            nodes,
        })
    }
...
pub enum DecoderCommand {
    ...
    /// Honor gAMA and cHRM chunks in PNG files. When false (potentially the
    /// future default), gAMA/cHRM are ignored and only iCCP/sRGB/cICP are used.
    /// Useful because gAMA/cHRM metadata is often incorrect in web-scraped images.
    ///
    /// Hidden from the public API — internal use and tests only.
    #[doc(hidden)]
    #[serde(rename = "honor_gama_chrm", skip)]
    HonorGamaChrm(bool),
    ...
    /// Hidden from the public API — internal use and tests only.
    #[doc(hidden)]
    #[serde(rename = "honor_gama_only", skip)]
    HonorGamaOnly(bool),
...
impl Response001 {
    ...
    pub fn example_ok() -> Response001 {
        Response001 { code: 200, success: true, message: None, data: ResponsePayload::None }
    }

    pub fn example_job_result_encoded(
        io_id: i32,
        w: i32,
        h: i32,
        mime: &'static str,
        ...
        let frame_perf = FramePerformance {
            nodes: vec![NodePerf { wall_microseconds: 30_000, name: "decode".to_owned() }],
            overhead_microseconds: 100,
            wall_microseconds: 30_100,
        };
        Response001 {
            code: 200,
            success: true,
...
fn decode_graph() {
    ...
    let expected = Graph {
        ...
        edges: vec![Edge { from: 0, to: 1, kind: EdgeKind::Input }],
    };
...
fn error_from_string() {
    ...
    assert_eq!(
        msg,
        "Error(\"invalid type: string \\\"hi\\\", expected i32\", line: 1, column: 18)"
    );
}
...
mod key_casing {
    ...
    #[allow(dead_code)]
    pub fn which_json_keys_cannot_roundtrip_casing<'de, T>(
        value: &T,
