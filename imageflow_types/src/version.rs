...
pub fn crate_parent_folder() -> std::path::PathBuf {
    ::std::path::Path::new(get_build_env_value("CARGO_MANIFEST_DIR").unwrap())
        .parent()
        .unwrap()
        .to_owned()
}
...
fn built_ago() -> (i64, &'static str) {
    let compiled_utc =
        ::chrono::DateTime::parse_from_rfc3339(benv::GENERATED_DATETIME_UTC).unwrap();
...
pub fn one_line_version() -> String {
    ...
    let target_cpu = match get_build_env_value("TARGET_CPU")
        .unwrap_or(get_build_env_value("RUSTFLAGS").unwrap_or("?"))
    {
        "x86-64" | "x86" => "",
    ...
    if ci && profile_release && release_tag.is_some() {
        ...
        format!(
            ...
            benv::GIT_DESCRIBE_ALWAYS,
            dirty_star(),
            branch.as_ref().unwrap_or(&"unknown branch"),
            benv::TARGET,
            target_cpu,
...
pub fn all_build_info_pairs() -> String {
    ...
    let mut pairs: Vec<(&&'static str, &Option<&'static str>)> =
        benv::BUILD_ENV_INFO.iter().collect();
    pairs.sort_by(|a, b| {
...
pub fn git_username_and_repo() -> &'static str {
    benv::BUILD_ENV_INFO
        .get("CI_REPO")
        .unwrap_or(&Some("imazen/imageflow"))
        .unwrap_or("imazen/imageflow")
}
