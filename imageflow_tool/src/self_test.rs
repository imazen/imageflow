...
//    }
    ...
    fn stderr_str(&self) -> &str{
        std::str::from_utf8(&self.r.stderr).expect("Implement lossy UTF-8 decoding for test results")
    }
    fn stdout_str(&self) -> &str{
        std::str::from_utf8(&self.r.stdout).expect("Implement lossy UTF-8 decoding for test results")
    }
    fn expect_exit_0_no_output(&self, m: &str) -> &ToolProduct{
        if self.stderr_bytes() > 0 || self.stdout_bytes() > 0 || self.status_code() != Some(0) {
            panic!("{}\nExpected exit code 0 and no output to stderr or stdout. Received {:?}", m, self.r);
        }
        &self
    }
    ...
    fn expect_stderr_contains(&self, substring: &str) -> &ToolProduct{
        if !self.stderr_str().contains(substring){
            panic!("Failed to locate substring {:?} within stderr output {}", substring, self.stderr_str());
        }
        self
    }
    ...
    fn expect_stdout_contains(&self, substring: &str) -> &ToolProduct{
        if !self.stdout_str().contains(substring){
            panic!("Failed to locate substring {:?} within stdout output {}", substring, self.stdout_str());
        }
        self
    }
...
impl TestContext{
    ...
    fn exec(&self, args: &str) -> ToolProduct{
        ...
        writeln!(&mut std::io::stderr(), "Testing {} {}", exe.to_str().unwrap(), args);
    ...
    fn create_blank(&self,filename_without_ext: &str, w: usize, h: usize, encoder: s::EncoderPreset){
...
pub fn run(tool_location: Option<PathBuf>) -> i32{
    ...
    }
    {
        ...
        let result = c.exec("v0.1/build --json example2.json --in 200x200.png 200x200.jpg --out out0.jpg");
...

    {
        ...
        c.write_json("bad__canvas_and_input_equal.json", s::Build001{builder_config: None, framewise: b.builder().to_framewise(), io: vec![]});
...

    {
        ...
        let g = s::Graph{
            edges: vec![s::Edge{from:0, to: 1, kind: s::EdgeKind::Input}, s::Edge{from:1, to: 0, kind: s::EdgeKind::Input}],
            nodes: nodes
        };
        c.write_json("bad__cycle.json", s::Build001{builder_config: None, framewise: s::Framewise::Graph(g), io: vec![]});
    ...
    }
    {
        ...
        nodes.insert("0".to_owned(), s::Node::FlipH);
        let g = s::Graph{
            edges: vec![s::Edge{from:0, to: 0, kind: s::EdgeKind::Input}],
            nodes: nodes
        };
        c.write_json("bad__loop.json", s::Build001{builder_config: None, framewise: s::Framewise::Graph(g), io: vec![]});
