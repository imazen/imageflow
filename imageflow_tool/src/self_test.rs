...
impl TestImageSource {
    pub fn get_bytes(&self) -> Result<Vec<u8>> {
        match *self {
            TestImageSource::Url(ref url) => ::imageflow_http_helpers::fetch_bytes(url)
                .map_err(|e| nerror!(ErrorKind::FetchError, "{}: {}", url, e))
                .map_err(|e| e.at(here!())),
            TestImageSource::Blank(ref blank) => Ok(blank.generate().bytes),
        }
    }
}
...
impl ReplacementOutput {
    pub fn file(io_id: i32, path: &'static str) -> ReplacementOutput {
        ReplacementOutput { io_id, value: OutputDestination::File { path: path.to_owned() } }
    }
    pub fn b64(io_id: i32) -> ReplacementOutput {
        ReplacementOutput { io_id, value: OutputDestination::Base64 }
    }

    pub fn parameter(&self) -> String {
        match self.value {
            OutputDestination::File { ref path } => {
                format!("{} {}", self.io_id, path)
            }
...
impl BuildScenario {
    ...
    fn prepare_scenario(&self, context: &ProcTestContext) -> Result<ProcTestContext> {
        ...
        let mut command =
            format!("{} v1/build --json {}", c.bin_location().to_str().unwrap(), json_fname);
...
fn scenario_export_4() -> BuildScenario {
    ...
    let framewise = v1
        .encode(1, preset.clone())
        .builder()
        .with(v2.encode(2, preset.clone()))
        .with(v3.encode(3, preset.clone()))
        .with(v4.encode(4, preset.clone()))
        .to_framewise();
    ...
    BuildScenario {
        ...
        new_outputs: vec![
            ReplacementOutput::file(1, "waterhouse_w1600.jpg"),
...
fn scenario_laundry_list() -> BuildScenario {
    let chain = fluent::fluently()
        ...
        .to(s::Node::RegionPercent {
            x1: -1f32,
            y1: -1f32,
            x2: 101f32,
            y2: 101f32,
            background_color: s::Color::Transparent,
        })
        .to(s::Node::Region {
            x1: -1,
            y1: -1,
    ...
    BuildScenario {
        ...
        recipe: framewise.wrap_in_build_0_1(),
        new_inputs: vec![ReplacementInput::File {
            path: "blank3200.jpg".to_owned(),
            source: TestImageSource::Blank(BlankImage {
                w: 3200,
                h: 3200,
                color: s::Color::Black,
                encoding: s::EncoderPreset::libjpeg_turbo_q(Some(5)),
            }),
        }],
...
pub fn run(tool_location: Option<PathBuf>) -> i32 {
    let c = ProcTestContext::create_timestamp_subdir_within(
        std::env::current_dir().unwrap().join("self_tests"),
        tool_location,
    );
    ...
    }
    {
        let recipe = fluent::fluently()
            .decode(0)
            .constrain_within(Some(60), Some(45), None)
            .encode(1, s::EncoderPreset::libjpeg_turbo())
            .into_build_0_1();
        c.write_json("example2.json", &recipe);
        c.create_blank_image_here("200x200", 200, 200, s::EncoderPreset::libjpeg_turbo());

        let result = c.exec("v0.1/build --json example2.json --in 200x200.jpg --out out3.jpg");
...

    {
        ...
        let g = s::Graph {
            edges: vec![
                s::Edge { from: 0, to: 1, kind: s::EdgeKind::Input },
                s::Edge { from: 1, to: 0, kind: s::EdgeKind::Input },
            ],
            nodes,
        };
        let recipe =
...
pub fn test_capture(tool_location: Option<PathBuf>) -> i32 {
    ...
    // encapsulate scenario/example for reuse
    {
        ...
        c.subfolder_context(Path::new("bundle_example_1b"))
            .exec("--capture-to recipe v1/build --json recipe.json --response response.json")
            .print_to_err()
            .expect_status_code(Some(0));
    }
    {
        let recipe = s::Build001::example_with_steps();
        c.write_json("example1.json", &recipe);
