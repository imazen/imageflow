        ...
        .subcommand(Command::new("v1/build").alias("v0.1/build")
            ...
            .arg(
                Arg::new("in").long("in").num_args(1..)
                    ...
                    .help("Replace/add inputs for the operation file. Use '-' to read the image from stdin")
            )
            .arg(Arg::new("out").long("out").action(clap::ArgAction::Append).num_args(1..)
                ...
                .help("Replace/add outputs for the operation file. Use '-' to write the image to stdout (the JSON response is then only available via --response)"))
        ...
        .subcommand(Command::new("v1/querystring").aliases(["v0.1/ir4","v1/ir4"])
            ...
            .arg(
                Arg::new("in").long("in").num_args(1..)
                    ...
                    .value_hint(ValueHint::FilePath)
                    .value_names(["source-image.jpg", "source-image-2.png"])
                    .help("Input image. Use '-' to read the image from stdin")
            )
            .arg(Arg::new("out").long("out")
                ...
                .value_hint(ValueHint::FilePath)
                .value_names(["result-1.jpg"])
                .help("Output image. Use '-' to write the image to stdout (the JSON response is then only available via --response)"))
