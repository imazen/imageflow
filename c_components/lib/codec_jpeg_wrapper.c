...
void wrap_jpeg_error_exit(j_common_ptr codec_info)
{
...
void wrap_jpeg_setup_error_handler(j_common_ptr codec_info, struct wrap_jpeg_error_state * state, void * custom_state, wrap_jpeg_error_handler  error_handler)
{
...
void * wrap_jpeg_get_custom_state(j_common_ptr codec_info)
{
    /* codec_info->err really points to a wrap_jpeg_error_state struct, so coerce pointer */
    struct wrap_jpeg_error_state *state = (struct wrap_jpeg_error_state *) codec_info->err;
...
bool wrap_jpeg_read_header(j_decompress_ptr codec_info){
    ...
    jpeg_read_header(codec_info, TRUE);
    return true;
}
