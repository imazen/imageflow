...
struct flow_Renderer * Renderer_create(flow_c * context, struct flow_bitmap_bgra * source,
                                       struct flow_bitmap_bgra * canvas, struct flow_RenderDetails * details);
                     ...
                     const struct flow_RenderDetails * details, bool transpose, int call_number)
{
    ...
    struct flow_bitmap_float * buf
        = flow_bitmap_float_create(context, pSrc->w, buffer_row_count, scaling_format, false);
    if (buf == NULL) {
        return false;
    }
...
bool Renderer_perform_render(flow_c * context, struct flow_Renderer * r)
{
    ...
    */
    ...
    struct flow_bitmap_bgra * finalDest = r->canvas == NULL ? r->source : r->canvas;
