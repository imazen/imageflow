...
static bool load_checksums(flow_c * c, struct named_checksum ** checksums, size_t * checksum_count){
    ...
    if (list == NULL){
        ...
        while (true) {
            ...
            index++;
            if (index >= list_size){
                FLOW_error_msg(c, flow_status_IO_error, "Could not read in entire checksum file. Please increase list_size above %ul.", list_size);
                fclose(fp);
                return false;
            }
        }
...
static bool checksum_bitmap(flow_c * c, struct flow_bitmap_bgra * bitmap, char * checksum_buffer, size_t checksum_buffer_length){
    char info_buffer[256];
    flow_snprintf(&info_buffer[0], sizeof(info_buffer), "%dx%d fmt=%d alpha=%d", bitmap->w, bitmap->h, bitmap->fmt, bitmap->alpha_meaningful);
    int64_t printed_chars= (flow_snprintf(checksum_buffer,checksum_buffer_length, "%016X_%016X", djb2_buffer((uint8_t *)bitmap->pixels, bitmap->stride * bitmap->h), djb2((unsigned const char *)&info_buffer[0])));
...
static bool append_html(flow_c * c, const char * name, const char * checksum_a, const char * checksum_b){
    ...
    if (checksum_b == NULL){
        ...
        fprintf(fp, "<h1>%s</h2>\n<img class=\"current\" src=\"%s.png\"/><img class=\"correct\" src=\"%s.png\"/><img class=\"diff\" src=\"compare_%s_vs_%s.png\"/>", name, checksum_a, checksum_b, checksum_a, checksum_b);
    }
