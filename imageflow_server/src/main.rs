...
fn execute_one_to_one<F>(source: &str, framewise_generator: F) -> std::result::Result<(stateless::BuildOutput, RequestPerf),ServerError>
...
fn respond_one_to_one<F>(source: &str, framewise_generator: F) ->  IronResult<Response> where F: Fn(s::ImageInfo) -> s::Framewise{
    match execute_one_to_one(source, framewise_generator) {
        Ok((output, perf)) => {
            let mime = output.mime_type.parse::<Mime>().unwrap_or(Mime::from_str("application/octet-stream").unwrap());

            Ok(Response::with((mime, status::Ok, output.bytes)))
        }
        Err(e) =>{
            let bytes =  format!("{:?}", e).into_bytes();
            Ok(Response::with((Mime::from_str("text/plain").unwrap(), status::InternalServerError, bytes)))
