...
fn parse_rgba_slices(
    ...
    b: &str,
    a: &str,
) -> Result<Color32, std::num::ParseIntError> {
    [a, r, g, b]
        .iter()
        .map(|s| {
            match s.len() {
        ...
        .try_fold(0u32, |acc, item| {
            if let Ok(v) = item {
                Ok(acc
                    .checked_shl(8)
                    .expect("4 8-bit shifts cannot overflow u32 when starting with zero")
                    | v)
            } else {
                item
            }
...
pub fn parse_color_hex(value: &str) -> std::result::Result<Color32, ParseColorError> {
    ...
    if let Err(u32_error) = u32_result {
        Err(ParseColorError::NotHexadecimal {
            desc: "Only hexadecimal colors are permitted here",
            parse_error: u32_error,
        })
    } else {
        let why =
            "Any substring of a valid hexadecimal string should also be a valid hexadecimal string";
        match value.len() {
