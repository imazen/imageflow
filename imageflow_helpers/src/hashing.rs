...
pub fn legacy_djb2(bytes: &[u8]) -> u64 {
    bytes
        .iter()
        .fold(5381u64, |hash, c| ((hash << 5).wrapping_add(hash)).wrapping_add(u64::from(*c)))
}
...
pub fn bits_select(hash: &[u8], from: usize, until: usize) -> Option<u64> {
    ...
    Some(
        (relevant_bytes.iter().fold(0u64, |acc, elem| u64::from(*elem) | (acc << 8))
            >> truncate_right)
            & mask,
    )
...
fn test_bits_format() {
    ...
    assert_eq!(
        "dbf90c29d914a7e3b0756e3365e87cf05723a7df53c01dcebda066ce7a99488c",
        bytes_to_hex(&hash)
    );
    assert_eq!(
        "/c/8/8c/88/dbf90c29d914a7e3b0756e3365e87cf05723a7df53c01dcebda066ce7a99488c",
        bits_format(&hash, "/{252-256:x}/{248-252:x}/{248-256:x}/{244-252:x}/{0-256:064x}")
    );
    assert_eq!(
        "/0c/22/4/dbf90c29d914a7e3b0756e3365e87cf05723a7df53c01dcebda066ce7a99488c",
        bits_format(&hash, "/{250-256:02x}/{244-250:02x}/{240-244:x}/{0-256:x}")
    );
}
