...
impl ProcOutput {
    ...
    pub fn stderr_str(&self) -> &str {
        std::str::from_utf8(self.stderr()).expect("Implement lossy UTF-8 decoding for test results")
    }
    pub fn stdout_str(&self) -> &str {
        std::str::from_utf8(self.stdout()).expect("Implement lossy UTF-8 decoding for test results")
    }
    pub fn expect_exit_0_no_output(&self, m: &str) -> &ProcOutput {
        if self.stderr_byte_count() > 0
    ...
    pub fn expect_stderr_contains(&self, substring: &str) -> &ProcOutput {
        if !self.stderr_str().contains(substring) {
            panic!(
                "Failed to locate substring {:?} within stderr output {}",
                substring,
                self.stderr_str()
            );
        }
...
impl ProcTestContext {
    pub fn create_timestamp_subdir_within<P: AsRef<Path>>(
        parent_folder: P,
    ...
    pub fn create<P: AsRef<Path>>(working_dir: P, exe_path: Option<PathBuf>) -> ProcTestContext {
        ...
        ProcTestContext { exe: self_path, working_dir: working_dir.as_ref().to_owned() }
    }

    pub fn copy_ancestral_file_here<P: AsRef<Path>>(
        &self,
        filename: P,
        ...
        F: Fn(&mut std::process::Child),
    {
        ...
        let _ = writeln!(&mut std::io::stderr(), "exit code {:?}", output.status.code());
        ...
        if output.status.code().is_none() && valgrind_on_signal_death {
            ...
            if std::env::var("VALGRIND_RUNNING").is_ok() {
                ...
                let _ =
                    writeln!(&mut std::io::stderr(), "Starting valgrind from within self-test:");
                let mut cmd = Command::new("valgrind");
                cmd.arg("-q").arg("--error-exitcode=9").arg(exe);
                cmd.args(args_vec.as_slice())
                    .current_dir(dir)
                    .env("RUST_BACKTRACE", "1")
                    .env("VALGRIND_RUNNING", "1");

                let _ = writeln!(&mut std::io::stderr(), "{:?}", cmd);
