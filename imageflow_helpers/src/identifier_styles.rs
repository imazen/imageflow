...
pub fn transform(s: &str, transform: Transform) -> String {
    match transform {
        Transform::AddUnderscores => {
            let temp = Regex::new("(?P<lookbehind>[^xy])([0-9]+)")
                .unwrap()
                .replace_all(s, "${lookbehind}_$2");
        ...
        Transform::Capitalize => Regex::new(r"(_|\b)([a-z])")
            .unwrap()
            .replace_all(s, |c: &Captures| c[0].to_uppercase())
            .into_owned(),
        Transform::LowerFirst => Regex::new(r"(\A|\s+)([A-Z])")
            .unwrap()
...
fn test_styling() {
    ...
    assert_eq!(
        "struct a_Imageflow_A_B_3so_10_A_2",
        transform("struct aImageflowAB3so10A2", Transform::AddUnderscores)
    );
}
