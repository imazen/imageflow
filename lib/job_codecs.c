...
static bool png_decoder_load_color_profile(flow_context* c, struct flow_job_png_decoder_state* state){
    ...
    //We assume that the underlying buffer can be freed after opening the profile, per http://www.littlecms.com/1/jpegemb.c
    ...
    if (profile == NULL && is_color_png &&
        !png_get_valid(state->png_ptr, state->info_ptr, PNG_INFO_sRGB) &&
        ...
        png_get_cHRM(state->png_ptr, state->info_ptr, &white_point.x, &white_point.y,
...
static bool flow_job_png_decoder_FinishRead(flow_context* c, struct flow_job_png_decoder_state* state)
{
    ...
    state->pixel_buffer_row_pointers = create_row_pointers(c, state->pixel_buffer,state->pixel_buffer_size, state->rowbytes, state->h);
    if (state->pixel_buffer_row_pointers == NULL){
        flow_job_png_decoder_reset(c, state);
        state->stage = flow_job_png_decoder_stage_Failed;
        FLOW_error_return(c);
