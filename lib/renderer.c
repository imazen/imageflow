...
static bool Renderer_complete_halving(Context* context, Renderer* r)
{
    ...
    result = r->source->can_reuse_space ? HalveInPlace(context, r->source, divisor)
...
static bool ApplyConvolutionsFloat1D(Context* context, const Renderer* r, BitmapFloat* img, const uint32_t from_row,
                                     const uint32_t row_count, double sharpening_applied)
{
    ...
    if (r->details->sharpen_percent_goal > sharpening_applied + 0.01) {
        ...
        if (!BitmapFloat_sharpen_rows(context, img, from_row, row_count,
...
static bool ScaleAndRender1D(Context* context, const Renderer* r, BitmapBgra* pSrc, BitmapBgra* pDst,
                             const RenderDetails* details, bool transpose, int call_number)
{
    ...
    for (uint32_t source_start_row = 0; source_start_row < pSrc->h; source_start_row += buffer_row_count) {
        ...
        if (!BitmapFloat_pivoting_composite_linear_over_srgb(context, dest_buf, 0, pDst, source_start_row, row_count,
                                                             transpose)) {
            CONTEXT_add_to_callstack(context);
            success = false;
            goto cleanup;
...
cleanup:
    ...
    if (contrib != NULL)
        LineContributions_destroy(context, contrib);

    if (source_buf != NULL)
        BitmapFloat_destroy(context, source_buf);
    if (dest_buf != NULL)
        BitmapFloat_destroy(context, dest_buf);
    /// p->Stop("Free Contributions,FloatBuffers", true, false);

    return success;
}

static bool Render1D(Context* context, const Renderer* r, BitmapBgra* pSrc, BitmapBgra* pDst,
                     const RenderDetails* details, bool transpose, int call_number)
{
    ...
    for (uint32_t source_start_row = 0; source_start_row < pSrc->h; source_start_row += buffer_row_count) {
        ...
        if (!BitmapFloat_pivoting_composite_linear_over_srgb(context, buf, 0, pDst, source_start_row, row_count,
                                                             transpose)) {
            CONTEXT_add_to_callstack(context);
            success = false;
            goto cleanup;
...
static bool RenderWrapper1D(Context* context, const Renderer* r, BitmapBgra* pSrc, BitmapBgra* pDst,
                            const RenderDetails* details, bool transpose, int call_number)
{
