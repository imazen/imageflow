...
#define IMAGEFLOW_ABI_VER_MINOR 2
...
//
// Safe to call from multiple threads on the same context (acquires write lock).
bool imageflow_context_add_output_buffer(void *context,
                                         int32_t io_id);
...
// * All void objects not explicitly freed
...
// * All pointers to data owned by this context become invalid (output buffers, void, etc.)
//
// # Panics
//
...
// ## Return Value
//
// Returns a pointer to a `void` on success or error. The response contains:
...
//
// The returned void is owned by the context and remains valid until:
...
// Frees a void object early.
//
// This is optional - void objects are automatically freed when the context is destroyed.
...
// Reads fields from a void and writes them to the provided output parameters.
//
// The buffer pointer will be a UTF-8 byte array (NOT null-terminated).
//
// **Important:** The buffer pointer is only valid until:
// * The void is freed with `imageflow_json_response_destroy`, OR
